...
library(sf)
library(terra)
setwd("D:/maxent/shredder/")
epsg=read.table("D:/maxent/shredder/layer/epsg_layer.txt",sep=";",header=T)
#############################
#      SET REGION           #
#############################

regions = c("AWT", "CAN", "NSW","NZ","SWI","SA")
for (region in regions){
  ...
  for (i in species){
    ...
    if(!dir.exists(paste0("samples/",region,"/",i,"/"))) dir.create(paste0("samples/",region,"/",i,"/"), recursive = T);
    sf::write_sf(pol, paste0("samples/",region,"/",i,"/",i,".gpkg"))
