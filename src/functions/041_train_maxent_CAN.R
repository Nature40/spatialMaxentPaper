...
foreach(i = 1:length(dirList)) %dopar% {
  ...
  folds = list.dirs(paste0(root_folder,"/samples/", region, "/", species, "/"), full.names = F, recursive = F)
  ...
  for (f in 8:length(folds)) {
    fold = folds[f]
    if(!file.exists(paste0(root_folder,"/output/",region,"/",species,"/", fold,"/",fold, "_results.csv"))){
      ...
      if(!file.exists(paste0(root_folder,"/output/",region,"/",species,"/",fold,"/spatial/final/",species,"_",species,".asc"))){
        # 1.0 spatial: ffs fvs tune beta and spatial cross-validation
        system(paste0("java -jar D:/spatialMaxent_jar/spatialMaxent.jar outputdirectory=", outDir,fold,"/spatial/ samplesfile=",root_folder,"/samples/",region,"/",species,"/",fold,"/",fold,"_train.csv ", shQuote("replicatetype=spatial crossvalidate")," ", shQuote("decisionParameter=test auc"), " environmentallayers=",root_folder,"/background/",region,"/", species,"_bg.csv warnings=false projectionLayers=",root_folder,"/layer/",region,"/",species,"/ outputGrids=false autorun writeMESS=false writeClampGrid=false askoverwrite=false togglelayertype=ontveg"
        ))
      }
      ...
      if(!file.exists(paste0(root_folder,"/output/",region,"/",species,"/",fold,"/tuned1/final/",species,"_",species,".asc"))){
        system(paste0("java -jar D:/spatialMaxent_jar/spatialMaxent.jar outputdirectory=", outDir,fold,"/tuned1/ samplesfile=",root_folder,"/samples/",region,"/",species,"/",fold,"/",fold,"_train.csv ", "replicates=5"," ", shQuote("decisionParameter=test auc"), " environmentallayers=",root_folder,"/background/",region,"/", species,"_bg.csv warnings=false outputGrids=false projectionLayers=",root_folder,"/layer/",region,"/",species,"/ autorun writeMESS=false writeClampGrid=false askoverwrite=false togglelayertype=ontveg"
        ))
      }
      ...
      if(!file.exists(paste0(root_folder,"/output/",region,"/",species,"/",fold,"/tuned2/final/",species,"_",species,".asc"))){
        system(paste0("java -jar D:/spatialMaxent_jar/spatialMaxent.jar outputdirectory=", outDir,fold,"/tuned2/ samplesfile=",root_folder,"/samples/",region,"/",species,"/",fold,"/",fold,"_train.csv ", "replicates=5"," ", shQuote("decisionParameter=test auc"), " environmentallayers=",root_folder,"/background/",region,"/", species,"_bg.csv ffs=true fvs=false outputGrids=false projectionLayers=",root_folder,"/layer/",region,"/",species,"/ warnings=false autorun writeMESS=false writeClampGrid=false askoverwrite=false togglelayertype=ontveg"
        ))
      }
      ...
      if(!file.exists(paste0(root_folder,"/output/",region,"/",species,"/",fold,"/standard/final/",species,"_",species,".asc"))){
        system(paste0("java -jar D:/spatialMaxent_jar/spatialMaxent.jar outputdirectory=", outDir,fold,"/standard/ samplesfile=",root_folder,"/samples/",region,"/",species,"/",fold,"/",fold,"_train.csv ", "replicates=5"," ", shQuote("decisionParameter=test auc"), " environmentallayers=",root_folder,"/background/",region,"/", species,"_bg.csv ffs=false fvs=false tuneBeta=false outputGrids=false projectionLayers=",root_folder,"/layer/",region,"/",species,"/ warnings=false autorun writeMESS=false writeClampGrid=false askoverwrite=false togglelayertype=ontveg"
        ))
      }
      ...
      for(folder in folderNames) {
        ...
        extr_po_df=na.omit(extr_po_df)
        
        AICC = resCSV$AICC[nrow(resCSV)]
        nParams = resCSV$Number.Parameters[nrow(resCSV)]
        
        # 2. calculate Boyce
        boyce=ecospat::ecospat.boyce(fit = r, obs = extr_po_df, nclass=0, window.w="default", res=100, PEplot = F, 
