#'@name 080_all_results.R
#'@author Lisa Bald [bald@staff.uni-marburg.de]
#'@description median values for each species of Forward Fold Metric Estimation  
...
FFME <- function(region = "SWI"){
  
  
  
  setwd("/home/bald/maxent/output/")
  
  outputList = list.files(file.path(envrmt$output,region), full.names=F, recursive = F)
  #outputList=outputList[-c(1)]
  
  results=lapply(1:length(outputList),  function(out){
    out2 = outputList[out]
    listFolds = list.files(file.path(envrmt$output, out2), full.names=F, recursive = T, pattern="results.csv")
    ...
    res = lapply(1:length(listFolds),  function(i){
      ...
      x= read.csv(file.path(envrmt$output, region,"/",out2, "/", listFolds[i]))
      x=t(x)
      x = as.data.frame(x)
      dat=x
      colnames(dat) <- dat[1,]
  ...
  write.csv(res2, file.path(envrmt$output, region,"/",region,"_results_all_folds.csv"))
